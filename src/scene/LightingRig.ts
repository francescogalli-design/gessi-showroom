...
export class LightingRig {
  ...
  constructor(scene: Scene) {
    // Key light - warm, upper right front (main modeling light)
    const keyLight = new DirectionalLight(new Color(0xfff5e8), 1.8);
    ...
    keyLight.shadow.radius = 3; // softer shadow edges
    scene.add(keyLight);
    this.lights.push(keyLight);

    // Fill light - cool blue, left side, softer
    const fillLight = new DirectionalLight(new Color(0xdce4f5), 0.4);
    fillLight.position.set(-0.35, 0.25, 0.15);
    scene.add(fillLight);
    this.lights.push(fillLight);

    // Rim/back light - creates silhouette edge highlight
    const rimLight = new DirectionalLight(new Color(0xffffff), 1.5);
    rimLight.position.set(-0.05, 0.35, -0.35);
    ...
    // Top spotlight - dramatic overhead pool of light on product
    const topSpot = new SpotLight(new Color(0xfff0d6), 2.5, 1.0, Math.PI / 7, 0.7, 1.5);
    topSpot.position.set(0, 0.6, 0.05);
    ...
    // Low grazing light - reveals surface texture and adds depth
    const grazingLight = new SpotLight(new Color(0xf0e8d8), 0.6, 0.8, Math.PI / 4, 0.9, 2);
